...
Boolean logic for policy enforcement.
Policy P: State -> {Allowed, Denied}.
Checks set membership (Tiers) and attribute matching (Tags).

Fault Detection Logic:
- Handles missing or malformed policy files.
- Returns explicit denial reasons.
- Default-allow or default-deny based on configuration (currently default-allow with checks).

Detailed Mechanisms:
1. Load policy JSON.
2. Check 'tier_gating': if enforced, verify operator tier is not in deny list.
3. Check 'privacy_enforced': if true, require compliance tags.
4. Check 'deterministic': if true, require operator determinism.
"""
